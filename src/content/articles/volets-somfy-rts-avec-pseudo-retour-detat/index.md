...
Voilà pour le virtuel, la commande **Etat** correspond à la valeur choisi pour l’ouverture du volet et la commande **Etat\_Save** correspond à la valeur précédente qui servira pour les calculs.
...
Avant de calculer le pourcentage de mouvement nous avons besoin de savoir si le volet allait vers le bas ou le haut. Pour vérifier cela on regarde si l’état précèdent (Etat\_Save) et supérieur ou inférieur à l’état choisi (Etat).
...
On met à jour la commande Etat\_save avec la valeur de la commande Etat pour le prochain lancement du scénario.
